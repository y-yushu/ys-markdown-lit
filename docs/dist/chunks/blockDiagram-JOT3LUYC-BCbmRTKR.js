import { _ as d, E as rt, d as O, e as oe, l as L, y as he, A as de, c as R, ah as ge, Q as ue, R as pe, ai as j, aj as Wt, ak as fe, u as $, k as xe, al as ye, am as xt, an as be, i as Tt } from "./index-BX-ZM0Ir.js";
import { c as we } from "./clone-DwsAL1rQ.js";
import { G as me } from "./graph-iD5y8-BI.js";
...
yt.parser = yt;
var Se = yt, X = /* @__PURE__ */ new Map(), St = [], bt = /* @__PURE__ */ new Map(), Bt = "color", Ct = "fill", ke = "bgFill", Pt = ",", ve = R(), ct = /* @__PURE__ */ new Map(), Ee = /* @__PURE__ */ d((e) => xe.sanitizeText(e, ve), "sanitizeText"), _e = /* @__PURE__ */ d(function(e, t = "") {
  let s = ct.get(e);
  ...
  for (const i of s) {
    ...
    if (i.type === "applyClass") {
      Ne(i.id, i?.styleClass ?? "");
      continue;
    }
    if (i.type === "applyStyles") {
      i?.stylesStr && De(i.id, i?.stylesStr);
      continue;
    }
...
var It = 0, Be = /* @__PURE__ */ d(() => (It++, "id-" + Math.random().toString(36).substr(2, 12) + "-" + It), "generateId"), Ce = /* @__PURE__ */ d((e) => {
  ...
  const s = Le, n = s(e, "r"), i = s(e, "g"), r = s(e, "b");
  return de(n, i, r, t);
...
function vt(e, t) {
  ...
  e.children.length > 0) {
    ...
    let a = e?.size?.x ? e?.size?.x + (-e?.size?.width / 2 || 0) : -C, l = 0;
    for (const c of e.children) {
      const u = e;
      if (!c.size)
        continue;
      const { width: h, height: x } = c.size, { px: w, py: b } = Ut(s, r);
      if (b != l && (l = b, a = e?.size?.x ? e?.size?.x + (-e?.size?.width / 2 || 0) : -C, L.debug("New row in layout for block", e.id, " and child ", c.id, l)), L.debug(
        `abc89 layout blocks (child) id: ${c.id} Pos: ${r} (px, py) ${w},${b} (${u?.size?.x},${u?.size?.y}) parent: ${u.id} width: ${h}${C}`
      ), u.size) {
        const S = h / 2;
        c.size.x = a + C + S, L.debug(
          `abc91 layout blocks (calc) px, pyid:${c.id} startingPos=X${a} new startingPosX${c.size.x} ${S} padding=${C} width=${h} halfWidth=${S} => x:${c.size.x} y:${c.size.y} ${c.widthInColumns} (width * (child?.w || 1)) / 2 ${h * (c?.widthInColumns ?? 1) / 2}`
        ), a = c.size.x + S, c.size.y = u.size.y - u.size.height / 2 + b * (x + C) + x / 2 + C, L.debug(
          `abc88 layout blocks (calc) px, pyid:${c.id}startingPosX${a}${C}${S}=>x:${c.size.x}y:${c.size.y}${c.widthInColumns}(width * (child?.w || 1)) / 2${h * (c?.widthInColumns ?? 1) / 2}`
        );
      }
      c.children && vt(c), r += c?.widthInColumns ?? 1, L.debug("abc88 columnsPos", c, r);
    }
  }
  L.debug(
...
var lr = /* @__PURE__ */ d((e, t) => {
  ...
  return e.forEach((r) => {
    if (!cr(t, r) && !i) {
      ...
      s.forEach((c) => {
        l = l || c.x === a.x && c.y === a.y;
      }), s.some((c) => c.x === a.x && c.y === a.y) || s.push(a), i = !0;
    } else
      n = r, i || s.push(r);
  }), s;
...
var wr = /* @__PURE__ */ d(async (e, t) => {
  ...
  const { shapeSvg: s, bbox: n } = await A(e, t, void 0, !0), i = 2, r = n.height + 2 * t.padding, a = r / i, l = n.width + 2 * a + t.padding, c = gr(t.directions, n, t), u = U(s, l, r, c);
  return u.attr("style", t.style), T(t, u), t.intersect = function(h) {
    return N.polygon(t, c, h);
  }, s;
  ...
  const { shapeSvg: s, bbox: n } = await A(e, t, Y(t), !0), i = n.width + t.padding, r = n.height + t.padding, a = [
    { x: -2 * r / 6, y: 0 },
    { x: i - r / 6, y: 0 },
    { x: i + 2 * r / 6, y: -r },
    { x: r / 6, y: -r }
...
var Rr = /* @__PURE__ */ d((e, t) => {
  ...
  const u = a.node().appendChild(K(c, t.labelStyle, !0, !0));
  ...
  const b = a.node().appendChild(
    K(x.join ? x.join("<br/>") : x, t.labelStyle, !0, !0)
  );
  if (j(R().flowchart.htmlLabels)) {
  ...
  let b = 0;
  const S = t.classData.annotations?.[0], E = t.classData.annotations[0] ? "«" + t.classData.annotations[0] + "»" : "", k = w.node().appendChild(K(E, t.labelStyle, !0, !0));
  let B = k.getBBox();
  if (j(R().flowchart.htmlLabels)) {
  ...
  const D = w.node().appendChild(K(_, t.labelStyle, !0, !0));
...
function _t(e, t, s = !1) {
  ...
  const c = be(n?.styles ?? []), u = n.label, h = n.size ?? { width: 0, height: 0, x: 0, y: 0 };
  return {
...
async function re(e, t, s) {
  ...
  if (s.getBlock(n.id).type !== "space") {
    const r = rt();
    await te(e, n, { config: r }), t.intersect = n?.intersect, Hr(n);
  }
}
